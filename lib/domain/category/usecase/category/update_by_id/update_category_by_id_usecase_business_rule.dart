...
class UpdateCategoryByIdUsecaseBusinessRule {
  ...
  const UpdateCategoryByIdUsecaseBusinessRule({
  ...
  Result<void, CategoryFailure> _entityValidation() {
    ...
    if (category.createdAt != null &&
        category.createdAt!.isAfter(DateTime.now())) {
      return const Result.failure(CategoryEntityInvalidCreatedAtFailure());
    }

    // lastTransaction
    if (category.lastTransaction != null &&
        category.lastTransaction!.isAfter(DateTime.now())) {
      return const Result.failure(
          CategoryEntityInvalidLastTransactionFailure());
    }

...
import 'package:baladeston/data/comment/error/comment_error_mapper.dart'
    as comment_error_mapper;
import 'package:baladeston/data/comment/filter/comment/comment_query_filter.dart';
import 'package:baladeston/data/comment/mapper/comment/comment_query_filter_mapper.dart';
import 'package:baladeston/data/comment/mapper/integrity/comment_data_integrity_mapper.dart';
...
import 'package:baladeston/domain/comment/failure/server/parsing/comment_model_parsing_failure.dart';
...
class CommentApiImplementation extends CommentApi {
  ...
  CommentApiImplementation({required this.client});

  Uri _url(String path, [Map<String, String>? query]) =>
      Uri.parse('${AppConfig.apiBaseUrl}/comment/$path')
          .replace(queryParameters: query);

  Result<T, CommentFailure> _handleDioError<T>(DioException e) {
    final errorData = DioErrorTranslator.translate(e);
    return Result.failure(
      comment_error_mapper.map(
        errorData.code,
  ...
  Result<T, CommentFailure> _parsingFailure<T>({
    required String debugMessage,
    String? rawSource,
  }) {
    return Result.failure(
      CommentModelParsingFailure(
  ...
  Result<T, CommentFailure> _handleParsingError<T>(
    Object e,
    StackTrace st,
    Object? raw,
  ) {
    ...
    if (kDebugMode) {
      debugPrint('⚠️ [CommentApi] Parsing error: $e\n$st');
    }
  ...
  void _logCorruptItem({
    ...
    if (kDebugMode) {
      debugPrint(
        '⚠️ [CommentApi] Corrupt comment item skipped: $reason\n$json',
      );
    }
  }
    ...
    try {
      ...
      if (map != null) {
        ...
        for (final e in itemsList) {
          ...
          final integrityFailure = CommentDataIntegrityMapper.map(e);
          ...
          try {
            models.add(CommentModel.fromJson(e));
        ...
        return Result.success(
          PaginatedResponseModel<CommentModel>(
            items: models,
  ...
  Future<Result<CommentModel, CommentFailure>> getCommentById({
    ...
    try {
      ...
      final integrityFailure = CommentDataIntegrityMapper.map(json);
      if (integrityFailure != null) {
        return Result.failure(integrityFailure);
      }

      return Result.success(CommentModel.fromJson(json));
    } on DioException catch (e) {
      return _handleDioError(e);
  ...
  Future<Result<CommentModel, CommentFailure>> createComment({
    ...
    Object? raw;
    try {
      raw = await client.post(_url(''), body: comment.toJson());
      final json = _asMap(raw);
  ...
  Future<Result<CommentModel, CommentFailure>> updateCommentById({
    ...
    Object? raw;
    try {
      raw = await client.put(_url('$id'), body: comment.toJson());
      final json = _asMap(raw);
